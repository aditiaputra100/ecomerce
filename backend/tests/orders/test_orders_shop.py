...
def test_prevent_self_purchase(client: TestClient):
    ...
    # 2. Login Seller
    login_res = client.post("/token", data={"username": "seller", "password": "password123"})
    ...
    shop_resp = client.post(
        ...
        data={"name": "Seller Shop", "description": "Primary store"}
    )
...
def test_shop_orders_and_status(client: TestClient):
    ...
    seller_login = client.post("/token", data={"username": "seller2", "password": "password123"})
    ...
    shop_resp = client.post(
        ...
        data={"name": "Seller2 Shop", "description": "Second store"}
    )
    assert shop_resp.status_code == 201, shop_resp.text
    ...
    buyer_login = client.post("/token", data={"username": "buyer2", "password": "password123"})
